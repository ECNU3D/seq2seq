...
# 		top_states = [array_ops.reshape(e,[-1,1,cell.output_size]) for e in encoder_cell]
# 		attention_states = array_ops.concat(top_states,1)
...
# 								beam_size=10
# 								):
# 	# create the output_size and the proj_biases
# 	if output_size is None:
# 		output_size = cell.output_size
# 	if output_projection is not None:
# 		proj_biases = ops.convert_to_tensor(output_projection[1],dtype = dtype)
# 		proj_biases.get_shape().assert_is_compatible_with([num_symbols])
# 	with variable_scope.variable_scope(scope or "embedding_attention_decoder",dtype=dtype) as scope:
# 		# the num_symbols is decoder_num_symbols
# 		embedding = variable_scope.get_variable("embedding", [num_symbols, embedding_size])
# 		emb_inp = [embedding_ops.embedding_lookup(embedding, i) for i in decoder_inputs]
# 		# create the loop_function to decode the output into the word space
# 		loop_function = _exact_beam_search(embedding,beam_size,num_symbols,embedding_size,outout_projection)
# 		# to decode
...
#     	# calculate the W_1 * h_t we use a 1-by-1 convolution, need to reshape before
#     	hidden = array_ops.reshape(attention_states,[-1,attn_length,1,attn_size])
#     	hidden_features = []
#     	v = []
#     	# size of the query vectors for attention
#     	attention_vec_size = attn_size
#     	for a in xrange(num_heads):
#     		k = variable_scope.get_variable("AttnW_%d" %a ,[1,1,attn_size,attention_vec_size])
#     		hidden_features.append(nn_ops.conv2d(hidden,k,[1,1,1,1,],"SAME"))
#     		v.append(variable_scope.get_variable("AttenV_%d" %a , [attention_vec_size]))

#     	state = []
#     	'''make the last hidden state of encoder become beam_size dimension
#     		because every time the batch_size of decoder is beam_size dimension
#     		initial_state is a index, corresponding to RNN, every element
#     		is LSTMTuple, including h and c
#     	'''
...
#         		for q in query_list:
#         		# check that n-dims == 2 if specified
...
#         outputs = []
#         prev = None
#         # attention redimed into beam_sized tensor
...
def _extract_beam_search(embedding, beam_size, num_symbols, embedding_size, output_projection=None):
    ...
    def loop_function(prev, i, log_beam_probs, beam_path, beam_symbols):
        ...
        # get the corresponding word embedding of beam-sized orders
